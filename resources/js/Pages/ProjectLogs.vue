    ...
    export default {
        ...
        methods: {
            loadItems(page = 1,length = 10){
                ...
                try{
                    $vm.loading = true;
                    axios.get($vm.route("time-logs.all"),{params: {page,length,project_id: $vm.project.id,start_date: $vm.form.start_date,end_date: $vm.form.end_date}}).then(({data}) => {
                        $vm.loading = false;
                        $vm.items = data.items;
            ...
            <div class="container">
                <div class="inner-content inner-content-users bg-white p-4">
                    <div class="d-flex gap-3 table-header align-items-center justify-content-between mb-4">
                        <h1 class="d-flex mb-0">{{ project.name }} Logs</h1>
                            ...
                            <tbody>
                                <tr v-if="items.data.length > 0 && !loading" v-for="item in items.data" :key="item.id">
                                    ...
                                    <td data-title="Action">
                                        <a class="btn btn-dark small-btn" title="View" @click="handlePopup(item)"><i class="fa fa-eye"></i> View</a>
                                    </td>
                                </tr>
