    ...
    export default {
        ...
        methods: {
            loadItems(page = 1,length = 10){
                ...
                try{
                    $vm.loading = true;
                    axios.get($vm.route("admin.project-logs.all"),{params: {page,length,project_id: $vm.project.id,...$vm.filters}}).then(({data}) => {
                        $vm.items = data.items;
            ...
            async exportLogs(){
                try{
                    const response = await axios.get(this.route('admin.project-logs.export'),{params: {project_id: this.project.id,...this.filters},responseType: "blob"});
