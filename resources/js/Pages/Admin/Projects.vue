    ...
    export default {
        ...
        methods: {
            loadItems(page = 1,length = 10){
                ...
                try{
                    $vm.loading = true;
                    axios.get($vm.route("admin.projects.all"),{params: {page,length}}).then(({data}) => {
                        $vm.loading = false;
                        $vm.items = data.items;
                    });
                }catch(e){
            ...
            async updateStatus(item){
                try{
                    ...
                    axios.post(this.route('admin.projects.status',item.id),{status: item.status}).then(({data}) => {
            ...
            actionHandler(item,type){
                if(type == "add"){
                    this.form = {id: '',name: '',client_id: ''};
                    this.addEditForm = true;
                }else if(type == "edit"){
                    this.form = {id: item.id,name: item.name,client_id: item.client_id};
                    this.addEditForm = true;
                }else if(type == "delete"){
                    this.confirmBoxForRecordDeletion(this.route('admin.projects.destroy',item.id));
                }
            },
            onSubmit(){
                if(this.submitting){
            ...
            handleValidate(){
                ...
                if(!this.form.client_id){
                    newError["client_id"] = "Required";
                    positionFocus = positionFocus || "client_id";
                }
            ...
            handleSubmit(){
                ...
                try{
                    document.getElementById("rt-custom-loader").style.display = "block";
                    axios.post($vm.route('admin.projects.add-update'),$vm.form).then(({data}) => {
                        document.getElementById("rt-custom-loader").style.display = "none";
                        toast(data.message,{"type": data.status,"autoClose": 3000,"transition": "slide"});
                        if(data.status == "success"){
                            $vm.form = {id: '',name: '',client_id: ''};
    ...
    <AdminLayout>
        <div class="bg-white p-4 rounded-30px ">
            <div class="d-flex gap-3 align-items-center justify-content-between mb-3">
                <h1 class="d-flex align-items-center gap-2"><span>Projects</span></h1>
                <button type="button" class="btn btn-dark btn-sm" @click="actionHandler(null,'add')">Add Project</button>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-if="items.data.length > 0 && !loading" v-for="item in items.data" :key="item.id">
                            ...
                            <td data-title="Client">{{ item.client_name }}</td>
    ...
    <div class="popup-wrapper active" v-if="addEditForm == true">
        <div class="inner-content">
            ...
            <div class="bg-white p-5 rounded-30px form-wrapper">
                <div class="d-flex gap-3 align-items-center justify-content-between mb-3">
                    <h1 class="d-flex">{{form.id ? "Update" : "Create"}} Project</h1>
                </div>
                <form @submit.prevent="onSubmit" class="row">
                    <div class="form-group col-sm-12">
                        <label for="name">Project Name</label>
                        <input type="text" class="form-control" :class="hasValidationError(errors,'name') ? 'has-cust-error' : ''" id="name" v-model="form.name" placeholder="Enter project name">
                        <div class="has-cust-error-msg" v-if="hasValidationError(errors,'name')">{{ validationError(errors,'name') }}</div>
                    </div>
                    <div class="form-group col-sm-12">
                        <label for="client_id">Client</label>
                        <select class="form-control" :class="hasValidationError(errors,'client_id') ? 'has-cust-error' : ''" id="client_id" v-model="form.client_id">
                            <option value="">Select</option>
                            <option v-for="client in clients" :key="client.id" :value="client.id">{{ client.name }}</option>
                        </select>
                        <div class="has-cust-error-msg" v-if="hasValidationError(errors,'client_id')">{{ validationError(errors,'client_id') }}</div>
                    </div>
                    <div class="col-sm-12">
                        <button type="submit" class="btn btn-dark mt-2 py-3">{{form.id ? "Update" : "Create"}} Project</button>
                    </div>
                </form>
            </div>
        </div>
