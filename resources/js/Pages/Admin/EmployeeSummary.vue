    ...
    export default {
        ...
        methods: {
            loadItems(page = 1,length = 10){
                ...
                try{
                    $vm.loading = true;
                    axios.get($vm.route("admin.employee-summary.all"),{params: {page,length,...$vm.filters}}).then(({data}) => {
                        $vm.items = data.items;
    ...
    <AdminLayout>
        <div class="bg-white p-4 rounded-30px">
            <div class="d-flex gap-3 header-bar align-items-center justify-content-between mb-4">
                <h1 class="mb-0">Employee Summary</h1>
                <div class="bar-form-wrapper gap-3 d-flex justify-content-between align-items-center">
                    <div class="field d-flex gap-2 align-items-center">
                        <label class="d-flex">Select Employee</label>
                        <select id="project-option" v-model="filters.user_id" class="form-control">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        <tr v-if="items.data.length > 0 && !loading" v-for="(item,index) in items.data" :key="index">
                            <td data-title="Employee Name">{{ item.user_name }}</td>
                            <td data-title="Project Name">{{ item.project_name }}</td>
                            <td data-title="Total Spent time">{{ convertMinutesToTime(item.time) }}</td>
                            <td data-title="Action">
                                <a class="btn btn-dark small-btn" title="View" :href="route('admin.employee-summary.view',[item.user_id,item.project_id])"><i class="fa fa-eye"></i> View</a>
                            </td>
                        </tr>
