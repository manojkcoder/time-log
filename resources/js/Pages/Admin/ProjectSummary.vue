    ...
    export default {
        ...
        methods: {
            loadItems(page = 1,length = 10){
                ...
                try{
                    $vm.loading = true;
                    axios.get($vm.route("admin.project-summary.all"),{params: {page,length,...$vm.filters}}).then(({data}) => {
                        $vm.items = data.items;
            ...
            <div class="d-flex gap-3 header-bar align-items-center justify-content-between mb-4">
                ...
                <div class="bar-form-wrapper gap-3 d-flex justify-content-between align-items-center">
                    <div class="field d-flex gap-2 align-items-center">
                        ...
                        <select id="project-option" v-model="filters.project_id" class="form-control">
                    ...
                    <tbody>
                        <tr v-if="items.data.length > 0 && !loading" v-for="item in items.data" :key="item.id">
                            ...
                            <td data-title="Action">
                                <a class="btn btn-dark small-btn" title="edit" :href="route('admin.project-summary.view',item.id)"><i class="fa fa-eye"></i> View</a>
                            </td>
                        </tr>
